...
float triangle(float x, float a) { 
	float output2 = 9.0*abs(  1.0*  ( (x/a) - floor( (x/a) + 0.5) ) ) - 1.0;
	return output2;
}
 
float field(in vec3 p) {	
	float strength = 9. + .03 * log(1.e-6 + fract(sin(time) * 373.11));
	float accum = 0.;
	float prev = 0.;
	float tw = 0.;	
...
void main() {   
	...
	float time2 = time;               
        float speed = -speed2;
        speed = .00005 * cos(time2*0.002 + 1.1415926/4.0);          
	//speed = 0.0;	
	...
	vec3 forward = vec3(4.,5.,1.);   
	from.x += transverseSpeed*(1.0)*cos(0.021*time) + 0.001*time;
	from.y += transverseSpeed*(0.0)*sin(0.01*time) +0.001*time;
	from.z += 0.003*time;	
