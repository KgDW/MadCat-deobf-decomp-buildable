...
float triangle(float x, float a) { 
	float output2 = 4.0*abs(  5.0*  ( (x/a) - floor( (x/a) + 0.5) ) ) - 1.0;
	return output2;
}
 
float field(in vec3 p) {	
	float strength = 7. + .09 * log(1.e-6 + fract(sin(time) * 373.11));
	float accum = 0.;
	float prev = 0.;
	float tw = 0.;	
...
void main() {   
     	vec2 uv2 = 4. * gl_FragCoord.xy / vec2(1024) - 1.;
	vec2 uvs = uv2 * vec2(1024)  / 1024.;
	
	float time2 = time;               
        float speed = -speed2;
        speed = .00005 * cos(time2*0.0002 + 3.1415926/2.0);          
	//speed = 0.0;	
	...
	//zoom
	float zooom = (time2-6311.)*speed;
	from += forward* zooom;
	float sampleShift = mod( zooom, stepsize );
	 
	float zoffset = -sampleShift;
	sampleShift /= stepsize; // make from 0 to 4
	...
	for (int r=0; r<volsteps; r++) {
		...
		if( r == 0 )
			fade *= (1.3 - (sampleShift));
		// fade in samples as they approach from the distance
		if( r == volsteps-1 )
			fade *= sampleShift;
		v+=vec3(s1,s1*s1,s1*s1*s1*s1)*a*brightness*fade; // coloring based on distance
		
		backCol2 += mix(.5, 1., v2) * vec3(0.8 * t3 * t3 * t3, 1.4 * t3 * t3, t3) * fade;
	...
	backCol2.b = 0.8*mix(backCol2.g, backCol2.b, 0.9);
	backCol2.g = 0.02;
	backCol2.bg = mix(backCol2.gb, backCol2.bg, 0.5*(cos(time*0.05) + 2.0));
