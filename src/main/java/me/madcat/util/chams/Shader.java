...
public abstract class Shader {
    ...
    public Shader(String string) {
        int n;
        int n2;
        this.fragmentShader = string;
        try {
            InputStream inputStream = this.getClass().getResourceAsStream("/shader/vertex.vert");
            n2 = this.createShader(IOUtils.toString(inputStream, Charset.defaultCharset()), 35633);
            IOUtils.closeQuietly(inputStream);
            InputStream inputStream2 = this.getClass().getResourceAsStream("/shader/" + string);
            n = this.createShader(IOUtils.toString(inputStream2, Charset.defaultCharset()), 35632);
