...
extends Command {
    ...
    public void execute(String[] commands) {
        ...
        if (commands.length >= 3) {
            switch (commands[0]) {
                case "save":
                    MadCat.configManager.saveConfig(commands[1]);
                    sendMessage(ChatFormatting.GREEN + "Config '" + commands[1] + "' has been saved.");
                    return;
                case "load":
                    if (MadCat.configManager.configExists(commands[1])) {
                        MadCat.configManager.loadConfig(commands[1]);
